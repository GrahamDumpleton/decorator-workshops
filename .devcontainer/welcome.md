# Welcome to the Python decorator workshops

These are guided, hands-on workshops on Python decorators: what a
decorator does, how to write one for functions, methods, classes and
coroutines, and why each of them works. Each one takes a question you
might have and has you answer it by doing it, in JupyterLab, with the
...
end of each offers the next.

Everything is Python and its standard library, so nothing is installed
for the workshops and the first page is ready as soon as JupyterLab is.
You never have to type code either. Every cell arrives by clicking the
action beside the step that explains it.
...
codespace, which here is to write a notebook and run cells in it, and
asks you how far to trust it. Choose Trust to let its actions run as
the workshop intends. Restricted asks before changing files or running
code. You are asked once for each workshop, and again only if it
changes. The workshops are not trusted for you, because the codespace
is yours, tied to your GitHub account. For the same reason JupyterLab
runs without the GitHub token the codespace holds for your account, so
nothing a workshop runs is handed it.
